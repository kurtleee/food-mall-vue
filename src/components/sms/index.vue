  ...
  <div class="app-container">
        ...
        <el-form :inline="true" size="small" label-width="140px">
          ...
          <el-form-item label="分拣状态：">
            <el-select v-model="paramData.param.status" @change="getList" class="input-width">
              ...
              <el-option label="已分拣" :value="1"></el-option>
              <el-option label="未分拣" :value="2"></el-option>
              <el-option label="分拣中" :value="3"></el-option>
            </el-select>
          </el-form-item>
    ...
    <div class="table-container">
      <el-table :data="list" :default-sort="{ prop: 'orderTime', order: 'descending' }">
        ...
        <el-table-column prop="sortTime" label="分拣时长" align="center" width="180"></el-table-column>
        <el-table-column prop="status" label="分拣状态" align="center">
          <template slot-scope="scope">
            <el-tag type="success" v-if="scope.row.status == 1">已分拣</el-tag>
            <el-tag type="danger" v-else-if="scope.row.status == 2">未分拣</el-tag>
            <el-tag type="warning" v-else>待分拣</el-tag>
          </template>
        </el-table-column>
    ...
    <el-footer style="position: fixed; bottom: 0; width: 100%; transition: opacity 0.8s ease-in-out !important;">
        <el-pagination
        v-if="showPagination"
...
export default {
  data() {
    return {
      ...
      paramData: {
        ...
        size: 10,
        param: {
          orderNumOrSorterNameOrRepositoryId: null,
          status: null,
          city: null,
          region: null,
          minOrderTime: null,
  ...
  methods: {
    ...
    getList() {
      ...
      this.$axios.post("api/sort/getSortList", this.paramData).then(res => {
